...
void TexWavesApp::AnimateMaterials(const GameTimer& gt)
{
	...
	// flare
	{
		auto box = mMaterials["wirefence"].get();

		XMMATRIX rot = XMMatrixRotationRollPitchYaw(0.f, 0.f, -10 * gt.DeltaTime());
		XMMATRIX transRotationOffset = XMMatrixTranslation(-0.5f, -0.5f, 0.f);
		XMMATRIX transRotationOffsetRev = XMMatrixTranslation(0.5f, 0.5f, 0.f);
		XMStoreFloat4x4(&box->MatTransform, XMLoadFloat4x4(&box->MatTransform) * transRotationOffset * rot * transRotationOffsetRev);
		box->NumFramesDirty = gNumFrameResources;
	}
}
...
void TexWavesApp::UpdateMaterialCBs(const GameTimer& gt)
{
	...
	for(auto& e : mMaterials)
	{
		...
		if(mat->NumFramesDirty > 0)
		{
			...
			matConstants.flareAlpha = mat->flareAlpha;
...
void TexWavesApp::LoadTextures()
{
	...
	fenceTex->Filename = L"../../Textures/flare.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
...
void TexWavesApp::BuildDescriptorHeaps()
{
	...
	auto flareTex = mTextures["flarealpha"]->Resource;
...
void TexWavesApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
    ...
    for(size_t i = 0; i < ritems.size(); ++i)
    {
		...
		if (ri->Geo->Name == "boxGeo")
		{
			CD3DX12_GPU_DESCRIPTOR_HANDLE tex(mSrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart());
			tex.Offset(3, mCbvSrvDescriptorSize);
